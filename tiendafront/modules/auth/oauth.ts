...
const urlAuth = (code_verifier: string) => {
...
const setVerifier = (code_verifier: string) => {
...
export const getVerifier = () => {
    ...
    let encrypted = localStorage.getItem('code_verifier');
    let data: string = JSON.stringify(encrypted).replace(/"/g,'');

   const decrypted = CryptoJS.AES.decrypt(data, enviroments.secret_pkce).toString(CryptoJS.enc.Utf8);
