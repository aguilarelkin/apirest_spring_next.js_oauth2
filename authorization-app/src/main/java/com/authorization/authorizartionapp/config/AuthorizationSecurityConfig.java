...
/*    @Bean
    ...
    public SecurityFilterChain webSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests((authorize) -> authorize
                        ...
                        .requestMatchers(/*"/auth/**", "/client/**",*/ "/login", "/api/v1/cliente/**", "/", "/oauth2/revoke", "/logout").permitAll()//client manejado por el administrador de sistemas
                ...
                .logout(lod -> {
                    // new OidcLogoutEndpointFilter - LogoutConfigurer - LogoutFilter - No HttpSession currently exists
                    lod.deleteCookies("JSESSIONID").invalidateHttpSession(true).clearAuthentication(true).logoutSuccessUrl("http://127.0.0.1:3000/logout").permitAll(true)
...
/*                .oauth2Login(oauth2Login ->
        ...
        http//agregar cuando no requiera csrf
                .csrf((csrf) -> csrf.ignoringRequestMatchers(/*"/auth/**", "/client/**",*/"/api/v1/cliente/**", "/oauth2/revoke", "/login", "/logout"));//.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
    ...
    public RegisteredClientRepository registeredClientRepository() {
        RegisteredClient oidcClient = RegisteredClient.withId(UUID.randomUUID().toString()).clientId("oidc-client")
                ...
                .clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC).authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE).authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN).authorizationGrantType(AuthorizationGrantType.CLIENT_CREDENTIALS)
                //.redirectUri("https://oauthdebugger.com/debug")
                .redirectUri("http://127.0.0.1:3000/authorized")
                //   .postLogoutRedirectUri("http://127.0.0.1:8080/")
                .scope(OidcScopes.OPENID).scope(OidcScopes.PROFILE).tokenSettings(TokenSettings.builder()
                        //.authorizationCodeTimeToLive(Duration.ofMinutes(2))
                        .accessTokenTimeToLive(Duration.ofHours(5)).build()).clientSettings(ClientSettings.builder().requireProofKey(true).build()).build();
